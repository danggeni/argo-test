...
import com.argo.test.config.KafkaProperties;
...
class HelloworldKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/helloworld-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/helloworld-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
